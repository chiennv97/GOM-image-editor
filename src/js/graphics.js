...
class Graphics {
    ...
    adjustCanvasDimension() {
        const canvasImage = this.canvasImage.scale(0.4);
    ...
    adjustCanvasDimensionZoom() {
        const canvasImage = this.canvasImage.scale(0.8);
    ...
    addImageObject(imgUrl) {
        ...
        return new Promise(resolve => {
            fabric.Image.fromURL(imgUrl, image => {
                ...
                image.set(consts.fObjectOptions.SELECTION_STYLE);
                image.set({
                    left: centerPos.x,
                    top: centerPos.y,
                ...
                image.type = 'image';
                image.imgUrl = imgUrl;
                this.getCanvas().add(image).setActiveObject(image);
    ...
    addImageObjectZoom(imgUrl, top, left, scaleX, scaleY) {
        // const callback = this._callbackAfterLoadingImageObject.bind(this);
    ...
    getObjectPosition(id, originX, originY) {
        ...
        const targetOrigin = targetObj.getPointByOrigin('left', 'top');
    ...
    getObjectPositionIcon(id, originX, originY) {
        const targetObj = this.getObject(id);
        if (!targetObj) {
            return null;
        }
    ...
    setObjectPositionFix(id, posInfo, scale, type) {
        // if (type === 1) {
        ...
        if (type === 'icon') {
            const iconComp = this.getComponent('ICON');
            if (scale === 2) {
                iconComp.add(targetObj.shape,
                    {fill: targetObj.fill,
                        top: y,
                        left: x,
        ...
        if (type === 'image') {
            if (scale === 2) {
                this.addImageObjectZoom(targetObj.imgUrl,
                    targetObj.top * 2, targetObj.left * 2, targetObj.scaleX * 2, targetObj.scaleY * 2);
            } else {
                this.addImageObjectZoom(targetObj.imgUrl,
                    targetObj.top / 2, targetObj.left / 2, targetObj.scaleX / 2, targetObj.scaleY / 2);
            }
        ...
        if (type === 'i-text') {
            const textComp = this.getComponent('TEXT');
            if (scale === 2) {
                textComp.add(targetObj.text,
                    {position: {x,
                        y},
